class Solution {
    public int[][] solution(int[][] arr1, int[][] arr2) { // int[][] -> 다차원배열(테이블을 만들어줌)
        ...
        final int ROW = arr1.length; // arr1과 arr2의 길이는 같기 때문에 arr1로만 설정해도 무관
        final int COL = arr1[0].length; // 오후시간에 발표 -> 어차피 0행의 열길이나 1행의 열길이나 똑같이때문에 0으로 설정/ 근데 왜 하필 0이냐? 어떤 행렬에는 3열0행처럼 0행만 있는 경우도 있을꺼니깐!
